...
def print_stats_json(*, jobs: int = 1) -> None:
    ...
    source_code_stats = configure.generate_source_code_stats(
        basedir=basedir, marker=marker
    )
    logger.info("dump to json...")
    data = build.render_source_code_stats(
        source_code_stats=source_code_stats, basedir=basedir
    )
...
def load_render_config(*, basedir: pathlib.Path) -> SiteRenderConfig:
    ...
    default_config_yml = yaml.safe_load(
        pkg_resources.resource_string(_RESOURCE_PACKAGE, _CONFIG_YML_PATH)
    )
...
def main(*, jobs: int = 1) -> None:
    ...
    source_code_stats = configure.apply_exclude_list_to_stats(
        excluded_paths=excluded_paths, source_code_stats=source_code_stats
    )
    markdown_paths = configure.apply_exclude_list_to_paths(
        markdown_paths, excluded_paths=excluded_paths
    )
    render_jobs = configure.convert_to_page_render_jobs(
    ...
    rendered_pages = build.render_pages(
        page_render_jobs=render_jobs,
        source_code_stats=source_code_stats,
        site_render_config=config,
    )
