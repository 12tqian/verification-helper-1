...
class TestCPlusPlusListDependencies(unittest.TestCase):
    ...
    def test_success(self) -> None:
        ...
        with tests.utils.load_files(files=files) as tempdir:
            with tests.utils.chdir(tempdir):
                ...
                actual = sorted(
                    cplusplus.CPlusPlusLanguage().list_dependencies(
                        tempdir / "main.cpp", basedir=tempdir
                    )
                )
    ...
    @unittest.skipIf(
        ...
        "The path separator should be '/' for this test.",
    )
    def test_failure_with_backslash(self) -> None:
        ...
        with tests.utils.load_files(files=files) as tempdir:
            with tests.utils.chdir(tempdir):
                self.assertRaises(
                    Exception,
                    lambda: cplusplus.CPlusPlusLanguage().list_dependencies(
                        tempdir / "main.cpp", basedir=tempdir
                    ),
                )
