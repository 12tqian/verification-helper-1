...
class TestCPlusPlusBundlingUnit(unittest.TestCase):
    ...
    @unittest.skipIf(
        shutil.which("clang++") is None, "clang++ is required for this test"
    )
    def test_reject_clang(self) -> None:
        ...
        with tests.utils.load_files(files) as tempdir:
            with tests.utils.chdir(tempdir):
                bundler = cplusplus_bundle.Bundler(
                    iquotes=[tempdir], compiler="clang++"
                )
...
class TestCPlusPlusBundlingEndToEnd(unittest.TestCase):
        ...
        with tests.utils.load_files(files) as tempdir:
            with tests.utils.chdir(tempdir):
                ...
                subprocess.check_call(
                    ["g++", "-o", "a.out", "main.bundled.cpp"], stderr=sys.stderr
                )

                # run
                self.assertEqual(
                    subprocess.check_output([str(pathlib.Path("a.out").resolve())]),
    ...
    def test_complicated(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tempdir_dst_:
            ...
            with tests.utils.load_files_pathlib(
                library_files_1
            ) as tempdir_src_library_1:
                with tests.utils.load_files_pathlib(
                    library_files_2
                ) as tempdir_src_library_2:
                    with tests.utils.load_files_pathlib(test_files) as tempdir_src_test:
                        with open(tempdir_dst / "main.bundled.cpp", "w") as fh:
                            with contextlib.redirect_stdout(fh):
                                args = [
                                    "-I",
                                    str(tempdir_src_library_1),
                                    "-I",
                                    str(tempdir_src_library_2),
                                    str(tempdir_src_test / "test" / "main.cpp"),
                                ]
            ...
            subprocess.check_call(
                [
                    "g++",
                    "-o",
                    str(tempdir_dst / "a.out"),
                    str(tempdir_dst / "main.bundled.cpp"),
                ],
                stderr=sys.stderr,
            )
            ...
            # run
            self.assertEqual(
                subprocess.check_output([str(tempdir_dst / "a.out")], input=b"10\n"),
                ("55" + os.linesep).encode(),
            )
            self.assertEqual(
                subprocess.check_output([str(tempdir_dst / "a.out")], input=b"20\n"),
                ("6765" + os.linesep).encode(),
            )
            self.assertEqual(
                subprocess.check_output([str(tempdir_dst / "a.out")], input=b"30\n"),
    ...
    def test_standard_headers(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tempdir_dst_:
            ...
            # compile
            subprocess.check_call(
                ["g++", str(tempdir_dst / "main.bundled.cpp")], stderr=sys.stderr
            )
