...
def get_timestamp_string(path: pathlib.Path) -> str:
    ...
    timestamp = datetime.datetime.fromtimestamp(
        epoch, tz=system_local_timezone
...
def get_timestamp_string_of_past() -> str:
    ...
    timestamp = datetime.datetime(
        year=2000, month=1, day=1, tzinfo=system_local_timezone
    )
    return timestamp.strftime(TIMESTAMP_FORMAT)
        ...
        with tests.utils.load_files(files) as tempdir:
            with tests.utils.chdir(tempdir):
                ...
                with onlinejudge_verify.marker.VerificationMarker(
                    ...
                    self.assertEqual(
                        verify.main(paths, marker=marker).failed_test_paths, []
                    )
        ...
        with tests.utils.load_files(files) as tempdir:
            with tests.utils.chdir(tempdir):
                ...
                with onlinejudge_verify.marker.VerificationMarker(
                    ...
                    self.assertEqual(
                        verify.main(paths, marker=marker).failed_test_paths, paths
                    )
                with open(timestamps_path) as fh:
                    timestamps = json.load(fh)
    ...
    def test_timestamps(self) -> None:
        ...
        with tests.utils.load_files(files) as tempdir:
            ...
            with open(timestamps_path, "w") as fh:
                json.dump(
                    {
                        "not-updated.test.cpp": get_timestamp_string(
                            tempdir / "not-updated.test.cpp"
            ...
            expected_return = list(
                map(pathlib.Path, ["updated-failure.test.cpp", "new-failure.test.cpp",])
            )
            expected_timestamps = {
                "not-updated.test.cpp": get_timestamp_string(
                    tempdir / "not-updated.test.cpp"
                ),
                "updated-success.test.cpp": get_timestamp_string(
                    tempdir / "updated-success.test.cpp"
                ),
                "new-success.test.cpp": get_timestamp_string(
                    tempdir / "new-success.test.cpp"
                ),
            }
            ...
            with tests.utils.chdir(tempdir):
                with onlinejudge_verify.marker.VerificationMarker(
                    ...
                    self.assertEqual(
                        sorted(verify.main(paths, marker=marker).failed_test_paths),
                        sorted(expected_return),
                    )
